...
export default function init({
  ...
  let fb2Promise = fb1Promise.then(init);
  ...
  function init() {
    return new Promise(resolve2 => {
      let fb = fork(`${__dirname}/fork.js`, ['--filename=' + fastbootFilename]);

      fb.on('exit', (code, signal) => {
        if (code === 1) {
          throw new Error(`Failed to run your fastboot file. Code ${code} signal ${signal}`);
        }
      });
